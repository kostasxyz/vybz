...
- `docs/refs/theme-module.md` — Theme system reference: state model, persistence/bootstrap flow, CSS token contract, and the checklist for adding new templates.
...
- `SettingsView.tsx` — Appearance controls: theme mode, UI template selection, and UI font size
...
- Theme templates must define both light and dark variable blocks in `src/App.css`; terminal colors follow light/dark mode automatically via root-level CSS variables; see `docs/refs/theme-module.md` for the token contract and add-template checklist
