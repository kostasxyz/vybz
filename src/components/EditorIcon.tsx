...
const KNOWN_EDITORS: Record<string, (props: { size: number }) => ReactElement> = {
  zed: ({ size }) => (
  ...
  code: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 100 100" fill="none">
      <mask id="vsc-m" maskUnits="userSpaceOnUse" x="0" y="0" width="100" height="100">
...
const GenericEditorIcon = ({ size }: { size: number }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
export function EditorIcon({ cmd, iconUrl, size = 16 }: { cmd: string; iconUrl?: string; size?: number }) {
  if (iconUrl) {
    return <img src={iconUrl} width={size} height={size} alt="" style={{ objectFit: "contain" }} />;
  }
  const Icon = KNOWN_EDITORS[cmd];
  return Icon ? <Icon size={size} /> : <GenericEditorIcon size={size} />;
}
